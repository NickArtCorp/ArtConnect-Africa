...
try:
    ...
    logging.warning("email_service module not found - email functionality disabled")
    class DummyEmailService:
        def send_pending_approval_email(self, *args, **kwargs): pass
        def send_approval_email(self, *args, **kwargs): pass
        def send_rejection_email(self, *args, **kwargs): pass
        def send_admin_notification(self, *args, **kwargs): pass
        ...
        try:
            conn.execute(__import__("sqlalchemy").text("""
                ...
                WHERE gender IS NOT NULL
                  AND lower(trim(gender)) IN ('male','men','m','man','homme');
            """))
            conn.execute(__import__("sqlalchemy").text("""
                UPDATE users
                SET gender = 'Female'
                WHERE gender IS NOT NULL
                  AND lower(trim(gender)) IN ('female','women','f','woman','femme');
            """))
            conn.execute(__import__("sqlalchemy").text("""
                UPDATE users
                SET gender = NULL
...
ARTISTIC_SECTORS = [
    {"name": "Visual Arts", "name_fr": "Arts Visuels", "name_pt": "Artes Visuais", "name_es": "Artes Visuales", "name_ar": "الفنون البصرية", "name_sw": "Sanaa ya Kitelezi", "name_af": "Visuele Kunste", "name_mg": "Zanasainatsy Fahitsy", "name_ti": "ምስላዊ ሰንዳቅ", "name_so": "Farshaxanka Muuqaalka", "name_am": "ምስላዊ ጥበብ"},
    {"name": "Performing Arts", "name_fr": "Arts du Spectacle", "name_pt": "Artes Performáticas", "name_es": "Artes Escénicas", "name_ar": "فنون الأداء", "name_sw": "Sanaa ya Utendaji", "name_af": "Uitvoerings Kunste", "name_mg": "Zanasainatsy Fampisehoana", "name_ti": "ዋናታት ሰንዳቅ", "name_so": "Farshaxanka Adiga", "name_am": "ትግበራ ጥበብ"},
    {"name": "Music", "name_fr": "Musique", "name_pt": "Música", "name_es": "Música", "name_ar": "الموسيقى", "name_sw": "Muziki", "name_af": "Musiek", "name_mg": "Mozika", "name_ti": "ሙዚቃ", "name_so": "Muusiga", "name_am": "ሙዚቃ"},
    {"name": "Literature", "name_fr": "Littérature", "name_pt": "Literatura", "name_es": "Literatura", "name_ar": "الأدب", "name_sw": "Fasihi", "name_af": "Letterkunde", "name_mg": "Angonsy", "name_ti": "ሥርዓተ ጻሕፍተ", "name_so": "Xasuusta", "name_am": "ስነ-ጽሑፍ"},
    {"name": "Cinema & Audiovisual", "name_fr": "Cinéma & Audiovisuel", "name_pt": "Cinema & Audiovisual", "name_es": "Cine & Audiovisual", "name_ar": "السينما والسمعية البصرية", "name_sw": "Sinema & Sauti-Kitelezi", "name_af": "Bioskoop & Klank-Beeld", "name_mg": "Sinema & Feo-Feony", "name_ti": "ሲነማ & ድምጽ-ምስላዊ", "name_so": "Sinima & Codka-Sawirka", "name_am": "ሲኒማ & ድምጽ-ምስላዊ"},
    {"name": "Fashion & Design", "name_fr": "Mode & Design", "name_pt": "Moda & Design", "name_es": "Moda & Diseño", "name_ar": "الأزياء والتصميم", "name_sw": "Mitindo & Ubunifu", "name_af": "Mode & Ontwerp", "name_mg": "Fitafitan & Famboarisana", "name_ti": "ቅብእ & መልካም", "name_so": "Hantida & Sameynta", "name_am": "ዓመታዊ & ንድፍ"},
    {"name": "Craftsmanship", "name_fr": "Artisanat", "name_pt": "Artesanato", "name_es": "Artesanía", "name_ar": "الحرف اليدوية", "name_sw": "Utendaji wa Mikono", "name_af": "Handwerk", "name_mg": "Asa-tana", "name_ti": "ስራተ-ጋዜ", "name_so": "Shaqooyinka Gacanta", "name_am": "ሕብረተ-ሥራ"},
    {"name": "Digital Arts", "name_fr": "Arts Numériques", "name_pt": "Artes Digitais", "name_es": "Artes Digitales", "name_ar": "الفنون الرقمية", "name_sw": "Sanaa ya Kidijitali", "name_af": "Digitale Kunste", "name_mg": "Zanasainatsy Isan-kaody", "name_ti": "ዲጂታላዊ ሰንዳቅ", "name_so": "Farshaxanka Dijiital", "name_am": "ዲጂታል ጥበብ"},
    {"name": "Cultural Heritage", "name_fr": "Patrimoine Culturel", "name_pt": "Patrimônio Cultural", "name_es": "Patrimonio Cultural", "name_ar": "التراث الثقافي", "name_sw": "Urithi wa Kitamaduni", "name_af": "Kulturele Erfenis", "name_mg": "Lova Kolontsaina", "name_ti": "ባህላዊ ወርስ", "name_so": "Dhaxalka Dhaqanka", "name_am": "ባህላዊ ውርሰት"},
]
...
ARTISTIC_DOMAINS = {
    "Visual Arts": [
        ...
        {"name": "Multidisciplinary", "name_fr": "Multidisciplinaire"},
    ],
    "Performing Arts": [
...
async def require_creator_or_admin(user = Depends(get_current_user)):
    """Only personne_physique, personne_morale creators and admins can create content"""
    if user.get("role") not in ["personne_physique", "personne_morale", "admin"]:
        raise HTTPException(status_code=403, detail="Only content creators can perform this action")
    return user

async def require_partner_or_admin(user = Depends(get_current_user)):
    """Only partners and admins can access detailed stats (deprecated - use require_paid_partner)"""
    if user.get("role") not in ["partenaire", "admin"]:
...
def normalize_gender(value: Optional[str]) -> Optional[str]:
    """
    Platform rule: only Male/Female are allowed.
    Accept common variants and normalize them.
    """
    if value is None:
        return None
...
async def register(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    elif user_data.role == 'personne_morale':
        avatar = f"https://api.dicebear.com/7.x/buildings/svg?seed={user_id}&backgroundColor=64748b"
    else:  # personne_physique
        avatar = f"https://api.dicebear.com/7.x/initials/svg?seed={user_data.first_name}%20{user_data.last_name}"

    # Partner code generation is only for admin-created partners (not for registration)
    ...
    new_user = User(
        ...
        organization_name=user_data.organization_name if user_data.role in ('personne_morale', 'visitor') else None,
        employees_count=user_data.employees_count if user_data.role == 'personne_morale' else None,
        visitor_type=user_data.visitor_type if user_data.role == 'visitor' else None,
        partner_code=None,  # Only admins can create partners with codes
        is_verified=False,
        is_featured=False,
...
async def login(credentials: UserLogin, db: Session = Depends(get_db)):
    user = db.query(User).filter(func.lower(User.email) == func.lower(credentials.email)).first()
    ...
    if user.approval_status == "pending":
        ...
        raise HTTPException(
            status_code=403, 
            detail="Your account has been rejected. Please contact support for more information."
        )
...
async def partner_login(credentials: PartnerLogin, db: Session = Depends(get_db)):
    """Login for partners using their unique partner code"""
    # Case-insensitive lookup by converting input to uppercase
    user = db.query(User).filter(User.partner_code == credentials.partner_code.upper()).first()
    if not user:
...
async def get_partner_code(user = Depends(get_current_user), db: Session = Depends(get_db)):
    """Get the partner code for authenticated partner user"""
    if user.get('role') != 'partenaire':
        raise HTTPException(status_code=403, detail="Only partners can access this endpoint")
    
    partner = db.query(User).filter(User.id == user.get('id')).first()
    if not partner or not partner.partner_code:
...
async def regenerate_partner_code(user = Depends(get_current_user), db: Session = Depends(get_db)):
    """Regenerate partner code (if lost)"""
    if user.get('role') != 'partenaire':
        raise HTTPException(status_code=403, detail="Only partners can access this endpoint")
    ...
    # Generate new code (stored in uppercase)
...
async def create_partner(user_data: UserCreate, admin_user = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    if user_data.role != 'partenaire':
        raise HTTPException(status_code=400, detail="This endpoint is only for creating partner accounts")
    ...
    partner_code = secrets.token_urlsafe(16).upper()
    
    # Partner avatar
    avatar = f"https://api.dicebear.com/7.x/corporate/svg?seed={user_id}&backgroundColor=64748b"
    
    # Create partner account (auto-approved for admins)
    new_user = User(
        id=user_id,
        email=user_data.email.lower(),
        password=hash_password(user_data.password),
    ...
    # Send welcome email with partner code
    try:
...
async def track_profile_view(
    ...
    artist = db.query(User).filter(User.id == artist_id, User.role.in_(["personne_physique", "personne_morale"])).first()
    if not artist:
        raise HTTPException(status_code=404, detail="Artist not found")
...
async def create_post(post_data: PostCreate, user = Depends(require_creator_or_admin), db: Session = Depends(get_db)):
    pid = str(uuid.uuid4())
    new_post = Post(
        id=pid,
        author_id=user["id"],
...
async def toggle_like(post_id: str, user = Depends(require_creator_or_admin), db: Session = Depends(get_db)):
    p = db.query(Post).filter(Post.id == post_id).first()
    if not p: raise HTTPException(status_code=404, detail="Post not found")
...
async def create_comment(post_id: str, data: CommentCreate, user = Depends(require_creator_or_admin), db: Session = Depends(get_db)):
    p = db.query(Post).filter(Post.id == post_id).first()
    if not p: raise HTTPException(status_code=404, detail="Post not found")
...
async def send_msg(data: MessageCreate, user = Depends(get_current_user), db: Session = Depends(get_db)):
    mid = str(uuid.uuid4())
    sender_type = user.get("role", "personne_physique")  # Use user's actual role
...
async def delete_portfolio_item(item_type: str, item_id: str, user = Depends(require_creator_or_admin), db: Session = Depends(get_db)):
    db_user = db.query(User).filter(User.id == user["id"]).first()
    portfolio = dict(db_user.portfolio)
...
async def create_proj(data: ProjectCreate, user = Depends(require_creator_or_admin), db: Session = Depends(get_db)):
    pid = str(uuid.uuid4())
...
def get_cached_statistics(db: Session, query_type: str, country: str = None, city: str = None, sector: str = None, **kwargs) -> dict | None:
    ...
    if cache_record and datetime.utcnow() <= cache_record.expires_at:
...
# ============== STATISTICS ROUTES V2 (MULTI-LEVEL) ==============
...
@api_router.get("/statistics/v2/by-city/{country}/{city}")
...
@api_router.get("/statistics/v2/by-country/{country}/sector/{sector}")
async def get_statistics_by_sector(
    ...
    intra_african_total = db.query(Project).filter(
        ...
        or_(
            ...
            Project.collaborators.contains(sector)
        )
    ).count()
...
@api_router.get("/statistics/v2/timeline/{country}")
...
@api_router.get("/statistics/v2/compare")
async def compare_countries(
    ...
    country_list = [c.strip() for c in countries.split(",")][:5]
...
@api_router.get("/statistics/v2/countries-list")
async def get_countries_with_artists(db: Session = Depends(get_db)):
    """Get list of countries that have registered artists"""
    countries = db.query(
        User.country, 
        ...
        "countries": [
            {
                "name": c[0],
                "artist_count": c[1],
                "subregion": get_subregion_from_country(c[0])
            } for c in countries
        ]
    }
...
async def get_collaboration_statistics(sector: Optional[str] = None, user = Depends(require_paid_partner), db: Session = Depends(get_db)):
    ...
    user_query = db.query(User).filter(User.role.in_(["personne_physique", "personne_morale"]))
    if sector:
    ...
    for row in cg_domain_rows:
        ...
        artist_id_q = db.query(User.id).filter(
            User.role.in_(["personne_physique", "personne_morale"]), User.country == country,
...
async def get_statistics_overview(user = Depends(get_optional_user), db: Session = Depends(get_db)):
    ...
    total_artists = db.query(User).filter(User.role.in_(["personne_physique", "personne_morale"]), User.profile_tag == "artist").count()
    total_professionals = db.query(User).filter(User.role.in_(["personne_physique", "personne_morale"]), User.profile_tag == "professional").count()
    total_media = db.query(User).filter(User.role.in_(["personne_physique", "personne_morale"]), User.profile_tag == "media").count()
...
async def get_detailed_statistics(
    ...
    base_query = db.query(User).filter(User.role.in_(["personne_physique", "personne_morale"]))
    if sector:
