...
export function Navbar() {
  ...
  // If pending or rejected, treat as not logged in
  ...
  // Seuls les partenaires et admins voient le lien Statistiques
  const canSeeStats = isLoggedIn && (isPartenaire || isAdmin);

  // Seuls les créateurs de contenu et admins voient Feed et Projects
  const canSeeSocialFeatures = isLoggedIn && (isPersonnePhysique || isPersonneMorale || isAdmin);

  // Créateurs de contenu, admins ET visiteurs peuvent envoyer des messages
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden lg:flex items-center gap-6">
            ...
            {isLoggedIn ? (
              <>
                {/* Messages : Artistes, Admins ET Visiteurs */}
                {canSeeMessages && (
                  <Link
                    to="/messages"
                    className="text-sm font-medium hover:text-primary transition-colors flex items-center gap-1"
