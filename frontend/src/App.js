...
function ApprovalRoute({ children }) {
  const { user, token } = useAuthStore();
  const { t } = useLanguageStore();
  ...
  // If user hasn't loaded yet, show loading
  if (token && user === null) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="w-6 h-6 rounded-full border-2 border-primary border-t-transparent animate-spin" />
  ...
  // If user is pending approval, redirect to home with message
  if (user?.approval_status === 'pending') {
    toast.error(t?.auth?.awaitingApproval || 'Your account is awaiting approval. Please check your email.');
    return <Navigate to="/" replace />;
  }

  // If user is rejected, redirect to home with message
  if (user?.approval_status === 'rejected') {
    toast.error(t?.auth?.accountRejected || 'Your account has been rejected. Please contact support.');
    return <Navigate to="/" replace />;
  }
        ...
        <main>
          <Routes>
            ...
            <Route path="/projects" element={
              <ApprovalRoute><VisitorRestrictedRoute><Projects /></VisitorRestrictedRoute></ApprovalRoute>
            } />
            ...
            <Route path="/messages" element={
              <ApprovalRoute><ProtectedRoute><Messages /></ProtectedRoute></ApprovalRoute>
            } />
            <Route path="/messages/:id" element={
              <ApprovalRoute><ProtectedRoute><Messages /></ProtectedRoute></ApprovalRoute>
            } />
            <Route path="/settings" element={
              <ApprovalRoute><ProtectedRoute><Settings /></ProtectedRoute></ApprovalRoute>
            } />
            <Route path="/admin/approvals" element={
              <ApprovalRoute><ProtectedRoute><AdminApproval /></ProtectedRoute></ApprovalRoute>
            } />
