...
export const translations = {
  en: {
    ...
    auth: {
      ...
      creating: 'Creating...', createInstitution: 'Create Institution Account', createPartner: 'Create Partner Account',
      canChooseMultidisciplinary: 'You can choose multidisciplinary',
      chooseSingleDomain: 'Choose only one domain',
  ...
  fr: {
    ...
    auth: {
      ...
      canChooseMultidisciplinary: 'Vous pouvez choisir multidisciplinaire',
      chooseSingleDomain: 'Choisissez un seul domaine',
  ...
  pt: {
    ...
    auth: {
      ...
      country: 'País', city: 'Cidade', subregion: 'Sub-região', gender: 'Género',
      ...
      contactPersonName: 'Nome da Pessoa de Contacto',
      contactPersonEmail: 'Email da Pessoa de Contacto',
      contactPersonInfo: 'Pessoa de referência (optionnel) - O seu relais pode contactar esta pessoa para verificar as suas informações se não é bem conhecido.'
    },
    profile: {
...
es: {
    ...
    auth: {
      ...
      country: 'País', city: 'Ciudad', subregion: 'Subregión', gender: 'Género',
      ...
      contactPersonEmail: 'Email de la Persona de Contacto',
      contactPersonInfo: 'Persona de referencia (optionnel) - Su relé puede contactar a esta persona para verificar su información si no es bien conocido.'
    },
    profile: {
...
ar: {
    ...
    auth: {
      ...
      contactPersonEmail: 'بريد شخص الاتصال',
      contactPersonInfo: 'جهة اتصال مرجعية (اختيارية) - يمكن لمفوضك الاتصال بهذا الشخص للتحقق من معلوماتك إذا لم تكن معروفاً.'
    },
    profile: {
...
sw: {
    ...
    auth: {
      ...
      contactPersonEmail: 'Barua pepe ya Mtu wa Mawasiliano',
      contactPersonInfo: 'Mtu wa mawasiliano (hiari) - Mgombezi wako anuwai anaweza kuwasiliana na mtu huyu ili kuthibitisha taarifa zako ikiwa hujui vizuri.'
    },
    profile: {
...
af: {
    ...
    auth: {
      ...
      contactPersonName: 'Kontakpersoon Naam',
      contactPersonEmail: 'Kontakpersoon E-pos',
      contactPersonInfo: 'Verwysing kontakpersoon (opsioneel) - Jou direksiebestuur kan hierdie persoon kontak om jou inligting te verifieer as jy nie goed bekend is nie.'
    },
    profile: {
...
mg: {
    ...
    auth: {
      ...
      contactPersonName: 'Anaran\'ny Mpiandry Fifandraisana',
      contactPersonEmail: 'Email an\'ny Mpiandry Fifandraisana',
      contactPersonInfo: 'Fifandraisanana mpifandray (misy safidy) - Ny sandikilao dia afaka andi-mailaka ity olona ity handehandeha an\'ny ianao raha tsy mahazatra.'
    },
    profile: {
...
ti: {
    ...
    auth: {
      ...
      emailPlaceholder: 'contact@organisation.org',
      contactPersonName: 'ናይ ርክብ ሰባት ሽም',
      contactPersonEmail: 'ናይ ርክብ ሰባት ኢ-ሜይል',
      contactPersonInfo: 'ርክብ ሰባት (ምልዋጥ) - ናይ ሪላይኻ ዓውዲ እዚ ሰባት እዚ ነ ርክብ ይክእል እንተ ንጹህ ኣይነበርካ።'
    },
    profile: {
      portfolio: 'ፖርትፎሊዮ',
      about: 'ብዛዕባ',
...
so: {
    ...
    auth: {
      ...
      contactPersonName: 'Magaca Qofka Xidhiidhka',
      contactPersonEmail: 'Iimaylka Qofka Xidhiidhka',
      contactPersonInfo: 'Xidhiidhka tixraac (opshunal) - Relay-gaagu waxay xidhiidhkan qofkan si ay wax maraan taariikhda hadii aydaan caanka ahayn.'
    },
    profile: {
      portfolio: 'Portfolio',
      about: 'Ku saabsan',
...
am: {
    ...
    auth: {
      ...
      emailPlaceholder: 'contact@organisation.org',
      contactPersonName: 'የእውቂያ ሰው ስም',
      contactPersonEmail: 'የእውቂያ ሰው ኢሜይል',
      contactPersonInfo: 'የማመሳከር ዋነኛ ግንኙነት (አማራጭ) - የአካባቢዎ ተወካይ ሰዋስው ምንነትዎን ለማረጋገጥ ይህን ሰውን ማግኘት ይችላል።'
    },
    profile: {
      portfolio: 'ፖርትፎሊዮ',
      about: 'ስለ',
...
export const useLanguageStore = create((set, get) => ({
  ...
  // Helper to get translated labels from reference data objects
  getLabel: (item) => {
    if (!item) return '';
  ...
  toggleLanguage: () => {
    ...
    document.documentElement.dir = 'ltr';
    const langs = availableLanguages.map(l => l.code);
    const currentIndex = langs.indexOf(get().language);
    const nextIndex = (currentIndex + 1) % langs.length;
    get().setLanguage(langs[nextIndex]);
  }
}));
...
export const useAuthStore = create((set, get) => ({
  ...
  partnerLogin: async (partner_code) => {
    set({ isLoading: true, error: null });
    try {
      const response = await axios.post(`${API}/auth/partner-login`, { partner_code });
      const { token, user } = response.data;
      localStorage.setItem('aca_token', token);
      set({ user, token, isLoading: false });
      return { success: true, user };
    } catch (error) {
      const message = error.response?.data?.detail || 'Invalid partner code';
      set({ error: message, isLoading: false });
      return { success: false, error: message };
    }
  },
