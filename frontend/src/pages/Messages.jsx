...
const getTagLabel = (tag) => {
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          ) : conv.user.role === 'personne_morale' && (
                            <Badge variant="secondary" className="bg-blue-50 dark:bg-blue-950 text-blue-700 dark:text-blue-400 border-0 text-[10px] h-4 whitespace-nowrap">
                              {t.auth.organisation}
                            </Badge>
                          )}
                        </div>
                        {!isVisitor && (conv.user.sector || conv.user.domain) && (
                          <p className="text-xs text-muted-foreground truncate mb-1">
                            {conv.user.role === 'personne_morale'
                              ? `${conv.user.sector || ''}`
                              : `${conv.user.sector || ''}${conv.user.domain ? ' › ' + conv.user.domain : ''}`
                            }
                          </p>
                        )}
  ...
  {chatUser.role === 'visitor'
    ? (chatUser.visitor_type === 'organisation' 
        ? t.auth.organisation 
        : t.auth.individual)
    : chatUser.role === 'personne_morale'
      ? `Organisation • ${chatUser.sector || ''}`
      : chatUser.sector && chatUser.domain
        ? `${getTagLabel(chatUser.profile_tag)} • ${chatUser.sector} › ${chatUser.domain}`
        : chatUser.sector
          ? `${getTagLabel(chatUser.profile_tag)} • ${chatUser.sector}`
          : getTagLabel(chatUser.profile_tag) || ''
  }
</p>
                  </div>
